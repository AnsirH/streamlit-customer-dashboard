...
def show():
    ...
    try:
        # st.set_page_config 사용 시 오류 발생 가능성 추적
        debug_log("st.set_page_config 호출 시도 전")
        # st.set_page_config(page_title="고객 이탈 예측", layout="wide")
        debug_log("st.set_page_config 호출 후 (주석처리됨)")
...
# 호출 추적을 위한 래퍼 함수
def show_prediction():
    debug_log("show_prediction() 함수가 app.py에서 호출됨")
    show()
    debug_log("show_prediction() 함수 실행 완료")
