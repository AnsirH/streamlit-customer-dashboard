...
if uploaded_file:
    ...
    # 줄 별 입력 구성
    updated_values["이용 기간"] = rows[0][0].number_input("이용 기간 (개월)", min_value=0, value=safe_int(selected_row["이용 기간"]))
    updated_values["거주 도시 등급"] = rows[0][1].selectbox("거주 도시 등급 (1~3)", [1, 2, 3], index=safe_int(selected_row["거주 도시 등급"], 1, 1)-1)
    ...
    updated_values["등록된 기기 수"] = rows[1][1].number_input("등록된 기기 수", min_value=0, value=safe_int(selected_row["등록된 기기 수"]))
    updated_values["만족도 점수"] = rows[1][2].slider("만족도 점수 (1~5)", 1, 5, safe_int(selected_row["만족도 점수"], 1, 3))

    updated_values["배송지 등록 수"] = rows[2][0].number_input("배송지 등록 수", min_value=0, value=safe_int(selected_row["배송지 등록 수"]))
    ...
    updated_values["쿠폰 사용 횟수"] = rows[3][0].number_input("쿠폰 사용 횟수", min_value=0, value=safe_int(selected_row["쿠폰 사용 횟수"]))
    updated_values["주문 횟수"] = rows[3][1].number_input("주문 횟수", min_value=0, value=safe_int(selected_row["주문 횟수"]))
    updated_values["마지막 주문 후 경과일"] = rows[3][2].number_input("마지막 주문 후 경과일", min_value=0, value=safe_int(selected_row["마지막 주문 후 경과일"]))

    updated_values["캐시백 금액"] = rows[4][0].number_input("캐시백 금액", min_value=0, value=safe_int(selected_row["캐시백 금액"]))
    ...
    updated_values["선호 결제 방식"] = rows[4][2].selectbox("선호 결제 방식", ["Credit Card", "Debit Card", "Cash on Delivery", "COD", "E wallet", "UPI"], index=["Credit Card", "Debit Card", "Cash on Delivery", "COD", "E wallet", "UPI"].index(selected_row["선호 결제 방식"]))

    updated_values["성별"] = rows[5][0].selectbox("성별", ["Male", "Female"], index=["Male", "Female"].index(selected_row["성별"]))
    updated_values["선호 주문 카테고리"] = rows[5][1].selectbox("선호 주문 카테고리", ["Mobile", "Mobile Phone", "Laptop & Accessory", "Grocery"], index=["Mobile", "Mobile Phone", "Laptop & Accessory", "Grocery"].index(selected_row["선호 주문 카테고리"]))
