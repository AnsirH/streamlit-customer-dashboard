...
if uploaded_file:
    ...
    "앱 사용 시간 (시간)", min_value=0.0, value=safe_float(selected_row["앱 사용 시간"]))
    ...
    "주문금액 상승률 (%)", min_value=0.0, value=safe_float(selected_row["주문금액 상승률"])
)
    ...
    "캐시백 금액", min_value=0, value=int(safe_float(selected_row["캐시백 금액"], fallback=0)))
