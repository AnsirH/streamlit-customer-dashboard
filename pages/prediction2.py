...
class ChurnPredictor:
    ...
    def predict(self, input_df):
        ...
        try:
            ...
            try:
                ...
                # 특성 중요도 계산 (SHAP 또는 모델 기반)
                top_features, importance_values = self._compute_feature_importance(processed_df)
                
                # 예측 확률 보정 (신뢰도 높은 결과를 위해)
                # 1. 로그 스케일 변환 - 매우 낮은 값을 더 큰 값으로 확대
                log_proba = -np.log10(1 - raw_proba + 1e-10) / 10
                # 2. 시그모이드 변환 - 중간 값 강조
                def sigmoid_transform(x, k=15, x0=0.15):
                    return 1 / (1 + np.exp(-k * (x - x0)))
                
                # 3단계 보정 확률 계산
                base_proba = raw_proba  # 원본 확률
                amplified_proba = np.minimum(raw_proba * 3.0, 0.99)  # 증폭 확률 (3배, 0.99 한도)
                sigmoid_proba = sigmoid_transform(raw_proba)  # 시그모이드 변환 확률
                log_scaled_proba = log_proba  # 로그 스케일 변환 확률
                
                # 최종 확률 결정 (여러 방법 중 중간값 사용)
                all_probas = np.vstack([amplified_proba, sigmoid_proba, log_scaled_proba])
                median_proba = np.median(all_probas, axis=0)
                
                # 디버그: 원시 예측값 출력
                with st.expander("디버그: 예측 상세 정보"):
                    col1, col2 = st.columns(2)
                    with col1:
                        st.write("### 예측 결과")
                        st.write(f"**예측 클래스:** {'이탈' if y_pred[0] == 1 else '유지'}")
                        st.write(f"**원본 확률:** {raw_proba[0]:.4f} ({raw_proba[0]*100:.1f}%)")
                        st.write(f"**증폭 확률:** {amplified_proba[0]:.4f} ({amplified_proba[0]*100:.1f}%)")
                        st.write(f"**시그모이드 확률:** {sigmoid_proba[0]:.4f} ({sigmoid_proba[0]*100:.1f}%)")
                        st.write(f"**로그 스케일 확률:** {log_scaled_proba[0]:.4f} ({log_scaled_proba[0]*100:.1f}%)")
                        st.write(f"**최종 보정 확률:** {median_proba[0]:.4f} ({median_proba[0]*100:.1f}%)")
                    
                    with col2:
                        st.write("### 확률 변환 과정 시각화")
                        # 다양한 변환 방식을 시각화
                        prob_data = pd.DataFrame({
                            '변환 방식': ['원본', '증폭(3x)', '시그모이드', '로그 스케일', '최종(중간값)'],
                            '확률값': [raw_proba[0], amplified_proba[0], sigmoid_proba[0], log_scaled_proba[0], median_proba[0]]
                        })
                        fig = px.bar(prob_data, x='확률값', y='변환 방식', orientation='h',
                                    title="확률 변환 과정",
                                    labels={'확률값': '확률 (0-1)'},
                                    color='변환 방식',
                ...
                return y_pred, median_proba  # 최종 보정된 확률 반환
    ...
    def _compute_feature_importance(self, input_data):
        ...
        try:
            ...
            # 방법 1: 모델의 feature_importances_ 속성 활용 (XGBoost, RandomForest 등)
            ...
            # 방법 2: 선형 모델의 계수 활용 (Logistic Regression 등)
            ...
            # 방법 3: 기본 방법 - 특성을 동일한 중요도로 취급
            else:
                top_features = feature_names[:top_n]
                importance_values = [1/len(feature_names)] * top_n
            
            # 캐싱
            self.feature_importance_cache = dict(zip(top_features, importance_values))
            
            return top_features, importance_values
        ...
        if missing_fields:
            ...
            with st.spinner("예측 중..."):
                try:
                    ...
                    # 영향 요인 계산
                    top_features, importance_values = predictor.get_feature_importance()
                    ...
                    if top_features is not None:
                        ...
                        # 바 차트로 시각화
                        fig = px.bar(
                            x=list(readable_features.values()),
                            y=list(readable_features.keys()),
                            orientation='h',
                        ...
                        # 주요 피처별 설명
                        st.write("### 영향 요인 분석")
                        for i, (feature, importance) in enumerate(readable_features.items()):
                            if i >= 3:  # 상위 3개만 설명
                                break
    ...
    if st.button("🔍 극단적 이탈 위험 고객 테스트", help="매우 높은 이탈 위험을 가진 가상의 고객 데이터로 테스트합니다."):
        ...
        with st.spinner("극단적 케이스 예측 중..."):
            try:
                ...
                st.error(f"극단적 케이스 예측 중 오류 발생: {str(e)}")
                import traceback
                st.code(traceback.format_exc())
